...
import lesson from '../src/data/lessons/lesson-2026-03-09.json';
  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        lesson.title,
        orderNum,
        ['nakute mo ii desu', 'dont have to', 'Setsubun', 'onomatopoeia'],
        JSON.stringify(lesson.vocabulary),
        JSON.stringify(grammarForDb),
        lesson.practice_phrases.map(p => p.jp)  // Extract just the Japanese text
      ]
    );
