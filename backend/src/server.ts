...
// Compact session endpoint - summarize and clean up old messages
app.post('/api/sessions/:id/compact', checkPassword, async (req, res) => {
  try {
    ...
    if (messages.length < 10) {
      return res.status(400).json({ 
        error: 'Not enough messages to compact (minimum 10)',
        messageCount: messages.length 
      });
    }
    
    // Get the last 3 messages to keep
    const messagesToKeep = messages.slice(-3);
    const messagesToSummarize = messages.slice(0, -3);
    
    // Build conversation text for summarization
    const conversationText = messagesToSummarize.map((m: any) => 
      `${m.role === 'user' ? 'Student' : 'Teacher'}: ${m.content}`
    ).join('\n');
    
    // Generate summary using AI
    const summaryPrompt = `Summarize this Japanese practice conversation in 2-3 sentences. 
Focus on: what topics were discussed, what grammar/vocabulary was practiced, and any mistakes made.
...
${conversationText}`;
    ...
    // Build new lesson context with summary
    const existingContext = session.lesson_context || '';
    const newContext = `${existingContext}\n\n[COMPACTED HISTORY]: ${summary}`;
    
    // Update session with new context
    await pool.query(
      'UPDATE sessions SET lesson_context = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
      [newContext, sessionId]
    );
    
    // Delete old messages (keep last 3)
    await pool.query(
      'DELETE FROM messages WHERE session_id = $1 AND id < $2',
      [sessionId, messagesToKeep[0].id]
    );
    
    // Add system message about compaction
    await pool.query(
      'INSERT INTO messages (session_id, role, content) VALUES ($1, $2, $3)',
      [sessionId, 'assistant', `[Session compacted. Summary: ${summary}]`]
    );
    ...
    res.json({
      ...
      messagesAfter: messagesToKeep.length + 1, // +1 for the system message
      contextSaved: newContext.length
    });
    
  } catch (error) {
    console.error('Error compacting session:', error);
    res.status(500).json({ error: 'Failed to compact session' });
  }
});
